...
def main(seed, discount, path, gpu):
    with tf.device('/device:GPU:%s' % gpu):
        ...
        logger.configure(dir='./logs/maze2/%s/%s/' % (path, seed), format_strs=['csv'])
        ...
        f = open('./logs/maze2/%s/%s/params.txt' % (path, seed), 'w')
        f.write(str(kwargs))
        f.close()
        ...
        act.save("./logs/maze2/%s/%s/maze.pkl" % (path, seed))
        save_plot(path, seed)


def save_plot(path, seed):
    results = pu.load_results('./logs/maze2/%s/%s/' % (path, seed))
    r = results[0]
    plt.plot(r.progress.steps, r.progress["mean 100 episode reward"])
    plt.savefig('./logs/maze2/%s/%s/plot.png' % (path, seed))
